...
const updateCounter = () => {
    ...
    const totalElements = actionList.children.length - actionList.querySelectorAll(".completed").length;
    cardCounter.textContent = `Open tasks: ${totalElements}`;
};
...
actionList.addEventListener('click', (e) => {
    ...
    if (action === "toggle") {
        ...
        if (nextSibling) {
            // Target parent container executes swap: inserts current card BEFORE the next sibling's next sibling
